...
export function CategoryNav({
  ...
  return (
    <div className="sticky top-[92px] z-30 bg-[#F8F9FA]/95 dark:bg-[#09090B]/95 backdrop-blur-md py-3 mb-8 border-b border-zinc-200/90 dark:border-[#23232E] transition-colors">
      
      {/* Category Pills (Glovo / RnR clean style) with generous headroom to prevent clipping */}
      <div className="flex items-center gap-2.5 overflow-x-auto no-scrollbar py-2.5 px-1 -my-1">
        {MENU_DATA.categories.map(cat => {
          ...
          return (
            <motion.button
              ...
              className={`group px-4 py-2.5 rounded-2xl text-xs font-bold transition-all flex items-center gap-2.5 shrink-0 select-none cursor-pointer relative ${
                isActive
            ...
            >
              <div className={`w-7 h-7 rounded-xl flex items-center justify-center transition-all duration-300 group-hover:scale-115 group-hover:rotate-6 ${
                isActive 
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mt-2.5 pt-2.5 border-t border-zinc-200/70 dark:border-rnr-border">
        
        {/* Quick Tag Chips with safe headroom */}
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-2 px-1 -my-1 text-xs">
          <motion.button
            type="button"
            ...
            className={`group px-3.5 py-2 rounded-xl text-xs font-semibold transition-all shrink-0 flex items-center gap-1.5 cursor-pointer select-none ${
              activeTag === 'hit'
        ...
        <div className="relative w-full md:w-80 shrink-0 group/search">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2.5 rounded-xl text-xs bg-white dark:bg-[#121215] border border-zinc-200 dark:border-[#23232E] text-zinc-950 dark:text-white placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-glovo-yellow focus:shadow-md focus:shadow-amber-500/15 transition-all"
          />
          {searchQuery && (
            <motion.button
              type="button"

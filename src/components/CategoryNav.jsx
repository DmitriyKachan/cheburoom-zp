      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-2 sm:gap-3">
          ...
          {MENU_DATA.categories.map(cat => {
            ...
            return (
              <button
                ...
                className={`h-7.5 sm:h-8 px-2.5 sm:px-3 rounded-full text-[11px] sm:text-xs font-bold transition-all flex items-center gap-1 sm:gap-1.5 shrink-0 select-none cursor-pointer whitespace-nowrap ${
                  isActive
        ...
        <div className="relative w-36 sm:w-48 md:w-56 lg:w-64 shrink-0 group/search">
          ...
          <input
            ...
            className="w-full pl-7.5 sm:pl-8.5 pr-7 h-7.5 sm:h-8 rounded-full text-[11px] sm:text-xs bg-white dark:bg-[#121215] border border-zinc-200 dark:border-[#23232E] text-zinc-950 dark:text-white placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-glovo-yellow transition-all shadow-xs"
          />
          {searchQuery && (
            <button
              type="button"
